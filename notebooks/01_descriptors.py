...
def _(desc_df, mo):
    ...
    n_total = len(desc_df)
    mo.md(f"**Descriptors:** {desc_df.shape[1]} features &nbsp;|&nbsp; {n_valid}/{n_total} molecules fully computed")
    return
...
def _(desc_df, df):
    ...
    def fig1():
        ...
        axes[1].bar(range(len(missing_frac[missing_frac > 0])), missing_frac[missing_frac > 0].values)
...
def _(desc_df, mo):
    def fig2():
        ...
        ax.hist(np.log10(variances.clip(lower=1e-10)), bins=60, edgecolor="white", linewidth=0.3)
        ax.axvline(np.log10(low_var_threshold), color="red", linestyle="--", linewidth=1.2,
        ...
        return mo.vstack([
            fig,
            mo.md(f"**{n_low}** / {len(variances)} descriptors have variance < {low_var_threshold}"),
        ])
...
def _(df, mo):
    ...
    mo.vstack([
        fig,
        mo.md(f"**{(counts == 0).sum()}** functional groups absent in all molecules"),
    ])
